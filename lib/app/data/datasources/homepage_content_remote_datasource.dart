...
import 'package:img/app/data/models/content_banner_model.dart';
import 'package:img/app/data/models/homepage_content_model.dart';
...
class HomepageContentRemoteDataSourceImpl
  ...
  Future<PaginatedEntity<ContentBannerEntity>> getContentBanner() async {
    try {
      final response = await DioNetwork.appAPI.get('/content/banners');
      if (response.statusCode != null && response.statusCode! < 300) {
        final data = response.data is Map<String, dynamic>
            ? response.data as Map<String, dynamic>
            : Map<String, dynamic>.from(response.data as Map);
        return PaginatedModel<ContentBannerEntity>.fromJson(
          data,
          (json) => ContentBannerModel.fromJson(json),
        );
      }
      throw Exception(
